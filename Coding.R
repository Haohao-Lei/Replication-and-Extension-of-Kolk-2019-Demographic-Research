...
#####load the data
gender <- read_sav("Country_Year_V-Dem_SPSS_v7.1/V-Dem-DS-CY-v7.1.sav")

####cleaning the gender data
gender <- dplyr::select(gender, country_name, country_text_id, year, v2x_gender)

####load the fertility data
fertility <- read.table('fertility/tfrRR.txt',sep='', skip = 2, header=TRUE)
fertility2 <- read_csv("fertility/API_SP.DYN.TFRT.IN_DS2_en_csv_v2_4700605.csv")
...
newgender <- read_excel("Gender equality/HistoricalGenderEqualityIndex_Broad.xlsx")
...
######################################################################################################################################################
##load the adjTFR
adjTFR <- read.table('fertility/adjtfrRRbo.txt',sep='', skip = 2, header=TRUE)
##rename the colums
...
###############################################################################################################
#load the data
appendix <- read_excel("Appendix/appendix.xlsx")
